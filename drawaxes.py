...
同时，为非氢原子添加 grid unit 坐标标签，并将所有标签存储在单一标签对象中，
以便单独控制标签的显示和隐藏。

用法：
    drawaxes_gridunits sele
"""
...
def drawaxes_gridunits(sele, name="axesGrid", spacing=2.0):
    ...
    # 构造 CGO 以绘制坐标轴线（直线仅绘制在各方向的边界上）
    cgo_grid = [BEGIN, LINES]
    # 绘制平行于 X 轴的直线（固定 Y 和 Z 坐标）
    ...
    # 为非氢原子添加 grid unit 坐标标签
    # 所有标签存储在同一个伪原子对象中，名称为 f"{name}_labels"
    label_obj = f"{name}_labels"
    # 删除旧的标签对象（如果存在）
    cmd.delete(label_obj)
    # 仅针对选择中的非氢原子进行标签设置
    cmd.select("temp_labels_sel", f"{sele} and not elem H")
    heavy_model = cmd.get_model("temp_labels_sel")
    for atom in heavy_model.atom:
        ...
        # 使用相同的 object 名称，逐个添加伪原子，从而将所有标签存储到 label_obj 中
        cmd.pseudoatom(object=label_obj, pos=atom.coord, label=label_str)
    # 显示该标签对象中的所有标签
    cmd.show("labels", label_obj)
    
    print("已为 ligand heavy atoms 添加 grid unit 坐标标签，并存储在单一的标签对象中。")
